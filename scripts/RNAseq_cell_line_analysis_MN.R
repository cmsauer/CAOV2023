...
CL_info_s$MN_group <- factor(CL_info_s$MN_group, levels = c("low", "high"))
...
dds <- DESeqDataSetFromTximport(txi, CL_info_s, design = ~MN_group+subtype) #additive model
...
res_MN_HighVsLow <-  DESeq2::results(ddsObj, alpha = 0.05, name =  "MN_group_high_vs_low")
res_normal_vs_HGSOC <-  DESeq2::results(ddsObj, alpha = 0.05, name =  "subtype_normal_vs_HGSOC")

sum(res_MN_HighVsLow$padj < 0.05 ,na.rm = TRUE)
sum(res_MN_HighVsLow$padj < 0.05 & res_MN_HighVsLow$log2FoldChange >0,na.rm = TRUE)
#189 genes were differentially expressed in additive model,and 114 with log2FC>0
...
#2046 genes were differentially expressed in additive model,and 529 with log2FC>0

sig_DGE_genes <- as.data.frame(res_MN_HighVsLow) %>% 
...
###Add annotations
res_MN_HighVsLow_annot <- as.data.frame(res_MN_HighVsLow) %>% 
...
#contrast MN high vs low
ddsShrink <- lfcShrink(ddsObj, coef = "MN_group_high_vs_low")
...
filtTab %>%
  ...
  scale_colour_manual(values = c(CMSgrey, CMSblue), name = "FDR < 0.1") +
...
# ggsave("../figures/CAOV_paper_figure7b.pdf", height = 2.5, width = 3.5)
# write.csv(filtTab, "../source_data_figures/source_data_figure7b.csv", row.names = F)
...
fgseaResTidy %>% 
  ...
  geom_col(aes(alpha=padj), fill = CMSblue)  +
  ...
  labs(y="Normalized Enrichment Score",
       title="% MN cells - high vs. low \n(median split)") + 
  theme_minimal(6) +
  theme(axis.title.y = element_blank(),
        legend.position = "bottom",
        legend.key.size = unit(0.1, "in")) +
  guides(alpha =guide_legend(nrow=2))
# ggsave("../figures/CAOV_paper_figure7d.pdf", width = 3.5, height = 2.5)
# write.csv(fgseaResTidy, "../source_data_figures/source_data_figure7d.csv", row.names = F)
...
for (i in 1:nrow(plotting_data)) {
  ...
  # ggsave(paste0("../figures/enrichm_plots_rnaseq/", pw, "_addMod_MN.pdf"), height = 2, width = 2.75)
}
